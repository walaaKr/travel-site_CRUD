...
$package_query = "SELECT id, title, description, price, image, destination, duration_days FROM packages WHERE id = " . intval($package_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $update_query = "UPDATE packages SET 
            ...
            duration_days = '" . $conn->real_escape_string($duration_days) . "'
            WHERE id = " . intval($package_id);
...
<section style="padding: 4rem 2rem; min-height: 80vh;">
    <div style="max-width: 700px; margin: 0 auto;">
        <h1 style="text-align: center; margin-bottom: 2rem;">Edit Package</h1>
        ...
        <form method="POST" action="update.php?id=<?php echo htmlspecialchars($package_id); ?>" novalidate>
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" required aria-label="Package description"><?php echo htmlspecialchars($_POST['description'] ?? $package['description']); ?></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="duration_days" name="duration_days" aria-label="Duration in days"
                       value="<?php echo htmlspecialchars($_POST['duration_days'] ?? $package['duration_days']); ?>">
            </div>
            
            <div class="form-group">
                <label for="image">Image Filename</label>
                <input type="text" id="image" name="image" aria-label="Image filename"
                       value="<?php echo htmlspecialchars($_POST['image'] ?? $package['image']); ?>">
            </div>
            
            <div style="display: flex; gap: 1rem;">
                <button type="submit" class="btn btn-primary" style="flex: 1;">Update Package</button>
